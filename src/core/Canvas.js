...
findInputPinHit(mouseX, mouseY) {
  for (let i = 0; i < this.nodes.length; i++) {
    ...
    for (let pinIndex = 0; pinIndex < node.inputPins.length; pinIndex++) {
      ...
      console.log(`🎯 Checking input pin ${pinIndex} of node ${i} at (${x.toFixed(1)}, ${y.toFixed(1)}), mouse at (${mouseX.toFixed(1)}, ${mouseY.toFixed(1)})`);
...
handleMouseUp(x, y) {
  if (this.isConnecting) {
    ...
    if (target) {
      ...
      console.log("✅ CONNECTION CREATED:", fromNode.label, "→", toNode.label);
    } else {
